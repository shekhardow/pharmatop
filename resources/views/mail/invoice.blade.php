    ...
    <div class="flex justify-center items-center min-h-screen py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full bg-white shadow-lg rounded-lg p-8">
            <div class="text-center mb-6">
                ...
                <h3 class="text-gray-700 mb-4">Hello <?php echo !empty($name) ? $name : 'there'; ?>,</h3>
            </div>
            <div class="text-gray-700 mb-4">
                <p><?php echo !empty($msg) ? $msg : 'We hope this message finds you well.'; ?></p>
                <p>Order ID: {{ !empty($payment->order_id) ? $payment->order_id : 'Order Id' }}</p>
                <p>Total Amount: {{ ($payment->currency ?? '') . ' ' . number_format($payment->amount, 2) }}</p>
            </div>
